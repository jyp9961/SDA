...
class ImageDecoder(nn.Module):
    def __init__(self, emb_dim=100) -> None:
        super().__init__()
        self.proj = nn.Linear(in_features=emb_dim, out_features=14112)
        self.conv1 = nn.Conv2d(
            in_channels=32, out_channels=128, kernel_size=3, padding=1
        )
        self.relu = nn.ReLU()
...
class ImagePredictor(nn.Module):
    def __init__(self, encoder, emb_dim=100):
        super().__init__()
        self.encoder = encoder
        self.decoder = ImageDecoder(encoder.out_dim)
